...
if($count !== 1){
    ...
    if(empty($_POST['rememberme'])){
        ...
        //Store them in a cookie that expires in 15 days
        function f1($a, $b){
            $c = $a . "," . bin2hex($b);
            return $c;
        }
        ...
        (`authentificator1`, `authentificator2`, `user_id`, `expires`)
        VALUES
        ('$authentificator1', '$f2authentificator2', '$user_id', '$expiration')";
        $result = mysqli_query($link, $sql);
        if(!$result){
            echo  '<div class="alert alert-danger">There was an error storing data to remember your next time.</div>';  
        }else{
            echo "success";   
        }
    }
